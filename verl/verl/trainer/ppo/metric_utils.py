...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Restoration
    if 'num_tools' in batch.non_tensor_batch:
        metrics['restoration/num_tools/mean'] = np.mean(batch.non_tensor_batch['num_tools'])
        metrics['restoration/num_tools/max'] = np.max(batch.non_tensor_batch['num_tools'])
        metrics['restoration/num_tools/min'] = np.min(batch.non_tensor_batch['num_tools'])

        metrics['restoration/psnr/mean'] = np.mean(batch.non_tensor_batch['psnr'])
        metrics['restoration/ssim/mean'] = np.mean(batch.non_tensor_batch['ssim'])
        metrics['restoration/lpips/mean'] = np.mean(batch.non_tensor_batch['lpips'])
        metrics['restoration/maniqa/mean'] = np.mean(batch.non_tensor_batch['maniqa'])
        metrics['restoration/clipiqa/mean'] = np.mean(batch.non_tensor_batch['clipiqa'])
        metrics['restoration/musiq/mean'] = np.mean(batch.non_tensor_batch['musiq'])
        metrics['restoration/degradation_reward/mean'] = np.mean(batch.non_tensor_batch['degradation_reward'])
        metrics['restoration/format_reward/mean'] = np.mean(batch.non_tensor_batch['format_reward'])
        available_tools = [
        ...
        for tool in available_tools:
            metrics[f'tools/{tool}/mean'] = np.mean(batch.non_tensor_batch[tool])

        metrics['training/num_degradations/mean'] = np.mean(batch.non_tensor_batch['num_degradations'])
