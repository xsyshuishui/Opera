...
# 注意：不设置 PYTORCH_NPU_ALLOC_CONF，使用默认内存分配策略
# 之前设置 max_split_size_mb:512 会限制最大内存块大小，导致大图推理失败
...
TILED_INFERENCE_THRESHOLD = 1.6 * 1024 * 1024  # 2.4M pixels
# 分块大小（像素）
TILE_SIZE = 512
...
def run_single_model_tiled(model, input_tensor, device, tile_size=TILE_SIZE, tile_pad=TILE_PAD, scale=1):
    ...
    基于 4KAgent/executor/dehazing/tools/X-Restormer 的实现
    ...
    for y in range(tiles_y):
        for x in range(tiles_x):
            ...
            # 释放分块显存（不需要每个 tile 都同步，最后统一同步即可）
...
def run_pipeline_inference(models, pipeline, lq_tensor, gt_tensor=None, return_intermediates=False):
    """
    按 pipeline 顺序执行推理
        ...
        for model_name in pipeline:
            ...
            if 'sr' in model_name.lower():
                ...
                if current_h >= gt_h and current_w >= gt_w:
                    print(f"⚠ 跳过SR模型 {model_name}: 当前尺寸({current_w}x{current_h}) >= GT尺寸({gt_w}x{gt_h})")
                    continue
...
def _save_generic_format(category_summary, output_path):
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for category in sorted(category_summary.keys()):
            ...
            avg_maniqa = np.mean(metrics['maniqa']) if metrics['maniqa'] else None
...
def load_metrics_progress(results_dir):
    """
    加载指标计算进度
    ...
    Returns:
        dict: 已计算的指标进度 {save_path: metrics_record, ...}
    """
    progress_path = os.path.join(results_dir, "metrics_progress.json")
    if os.path.exists(progress_path):
        try:
            with open(progress_path, 'r', encoding='utf-8') as f:
                progress = json.load(f)
            print(f"✓ 加载指标进度文件: {len(progress)} 条记录")
            return progress
        except Exception as e:
            print(f"⚠ 加载指标进度文件失败: {e}，将重新计算所有指标")
            return {}
    return {}


def save_metrics_progress(results_dir, progress):
    """
    保存指标计算进度（原子写入，防止中断导致数据丢失）

    Args:
        results_dir: 结果目录
        progress: 进度字典 {save_path: metrics_record, ...}
    """
    progress_path = os.path.join(results_dir, "metrics_progress.json")
...
def calculate_all_metrics_api(pred_path, gt_path):
    url = f"http://127.0.0.1:6020/evaluate"
    payload = {
    ...
    try:
        ...
        resp = requests.post(url, json=payload, timeout=300)  # 5 分钟超时
        ...
        # 检查是否被跳过（大图片）
        if data.get("skipped"):
            print(f"[SCORE-API] Skipped: {data.get('reason', 'unknown')}")
            return False, None, f"Skipped: {data.get('reason', 'unknown')}"
        score = data
...
def main():
    ...
    # NPU 预热：用小图片初始化 NPU 算子，避免第一张图片 OOM
    print("\n预热 NPU...")
    try:
        warmup_tensor = torch.rand(1, 3, 64, 64).to(DEVICE)
        # 只预热一个模型即可初始化 NPU
        first_model = list(models.values())[0]
        with torch.no_grad():
            _ = first_model.net_g(warmup_tensor)
        del warmup_tensor, _
        if DEVICE.startswith('npu'):
            torch.npu.empty_cache()
            torch.npu.synchronize()
    ...
    # 在推理开始前清理缓存，确保有足够的连续内存
    import gc
    gc.collect()
    if DEVICE.startswith('npu'):
        torch.npu.empty_cache()
        torch.npu.synchronize()
        print(f"✓ 推理前清理 NPU 缓存完成")
    elif DEVICE.startswith('cuda'):
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
        print(f"✓ 推理前清理 CUDA 缓存完成")
    ...
    for pipeline_config in tqdm(config['pipelines'], desc="推理进度"):
        ...
        for data_item in tqdm(pipeline_config['data'], desc=f"  Pipeline {pipeline_id}", leave=False):
            ...
            intermediates = None
            max_retries = 2
            retry_occurred = False  # 标记是否发生了重试
                ...
                try:
                    if args.detail:
                        ...
                        pred_tensor, intermediates = run_pipeline_inference(
                            models, pipeline, lq_tensor, gt_tensor, return_intermediates=True
                        )
                    else:
                        pred_tensor = run_pipeline_inference(models, pipeline, lq_tensor, gt_tensor)
                        intermediates = None
                    break  # 成功，退出重试循环
                except Exception as e:
                    if retry < max_retries - 1:
                        # 还有重试机会，清理缓存后重试
                        print(f"⚠ 推理失败 (尝试 {retry + 1}/{max_retries}): {img_name}, 清理缓存后重试...")
                        retry_occurred = True
                        gc.collect()
                        if DEVICE.startswith('npu'):
                            torch.npu.empty_cache()
            ...
            # 如果发生过重试，在保存后进行更彻底的清理
            if retry_occurred:
                print(f"✓ 重试成功: {img_name}, 执行彻底清理...")
                gc.collect()
                if DEVICE.startswith('npu'):
                    torch.npu.empty_cache()
                    torch.npu.synchronize()
            ...
            if inference_count % 50 == 0 and inference_count > 0:
                if DEVICE.startswith('npu'):
                    torch.npu.empty_cache()
                elif DEVICE.startswith('cuda'):
                    torch.cuda.empty_cache()

        # 每个 pipeline 结束后清理缓存，减少内存碎片
        gc.collect()
        if DEVICE.startswith('npu'):
            torch.npu.empty_cache()
        elif DEVICE.startswith('cuda'):
    ...
    # 加载指标计算进度（断点续传）
    metrics_progress = load_metrics_progress(results_dir)
    metrics_skip_count = 0
    metrics_calc_count = 0
    ...
    step_metrics_all = defaultdict(lambda: {
        'psnr': [], 'ssim': [], 'lpips': [], 'clipiqa': [], 'musiq': [], 'maniqa': [],
        'psnr_delta': [], 'ssim_delta': [], 'lpips_delta': [],
        'clipiqa_delta': [], 'musiq_delta': [], 'maniqa_delta': [],
        'model_names': set()
    ...
    for item in tqdm(inference_results, desc="计算指标"):
        ...
        # 断点续传：检查是否已计算过指标
        # ============================================================
        if save_path in metrics_progress:
            # 已计算过，直接使用保存的结果
            cached = metrics_progress[save_path]
            metrics = cached['metrics']
            lq_metrics = cached.get('lq_metrics')
            step_metrics_list = cached.get('step_metrics', [])
            ...
            # 无参考指标 (CLIPIQA, MUSIQ, MANIQA) 始终累积
            ...
            # detail 模式下，累积到 step_metrics_all
            if args.detail and step_metrics_all is not None:
                # 累积 LQ 指标
                if lq_metrics:
                    if has_gt:
                    ...
                    if lq_metrics.get('maniqa') is not None:
                        step_metrics_all[0]['maniqa'].append(lq_metrics['maniqa'])
                    step_metrics_all[0]['model_names'].add('LQ (input)')

                # 累积中间步骤指标
                for step_info in step_metrics_list:
                    step_idx = step_info['step']
                    model_name = step_info['model_name']
                    inter_metrics = step_info['metrics']
                    ...
                    if inter_metrics.get('maniqa') is not None:
                        step_metrics_all[step_idx]['maniqa'].append(inter_metrics['maniqa'])
                    ...
                    if lq_metrics:
                        ...
                        if inter_metrics.get('maniqa') and lq_metrics.get('maniqa'):
                            step_metrics_all[step_idx]['maniqa_delta'].append(
                                inter_metrics['maniqa'] - lq_metrics['maniqa'])

            metrics_skip_count += 1
            continue
        ...
        metrics['psnr_y'] = None  # psnr_y 需要单独计算，API 返回的是 psnr
        ...
        metrics_progress[save_path] = {
            ...
            'has_gt': has_gt,
            'metrics': metrics,
            'lq_metrics': lq_metrics,
            'step_metrics': step_metrics_list
        }
        # 每计算完一张图片就保存进度
        save_metrics_progress(results_dir, metrics_progress)
        metrics_calc_count += 1

    print(f"\n✓ 指标计算完成: 新计算 {metrics_calc_count} 张, 跳过 {metrics_skip_count} 张 (已缓存)")
        ...
        if args.detail:
            ...
            for category in sorted(category_metrics.keys()):
                ...
                for img_info in cat_data['images']:
                    ...
                    f.write(f"\n  Image: {img_name}\n")
                    f.write(f"    Category: {img_info.get('category', category)}\n")
                    ...
                    if lq_metrics:
                        f.write(f"    [LQ (input)]\n")
                        f.write(f"      Path: {img_info['lq_path']}\n")
                        metrics_str = []
                        if lq_metrics.get('psnr') is not None:
                        ...
                        if lq_metrics.get('maniqa') is not None:
                            metrics_str.append(f"MANIQA={lq_metrics['maniqa']:.4f}")
                        if metrics_str:
                    ...
                    for step_info in step_metrics:
                        ...
                        if metrics.get('maniqa') is not None:
                            metrics_str.append(f"MANIQA={metrics['maniqa']:.4f}")
                        if metrics_str:
                            f.write(f"      {', '.join(metrics_str)}\n")
                    ...
                    f.write(f"      Path: {img_info['pred_path']}\n")
                    metrics_str = []
                    ...
                    if final_metrics.get('maniqa') is not None:
                        metrics_str.append(f"MANIQA={final_metrics['maniqa']:.4f}")
                    if metrics_str:
                        f.write(f"      {', '.join(metrics_str)}\n")
