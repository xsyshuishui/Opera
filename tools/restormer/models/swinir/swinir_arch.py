...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        # Cache for dynamically computed masks (to avoid recreating for same size)
        # Using a simple dict cache with size limit
        self._mask_cache = {}
        self._mask_cache_max_size = 2  # Only cache 2 sizes to limit memory
    ...
    def calculate_mask(self, x_size, device=None):
        ...
        # Using torch.no_grad() to prevent mask from being part of computation graph
        H, W = x_size
    ...
    def forward(self, x, x_size):
        ...
        if self.input_resolution == x_size:
            ...
            # Use cached mask if available, otherwise calculate and cache
            cache_key = (x_size[0], x_size[1], str(x.device))
            if cache_key in self._mask_cache:
                mask = self._mask_cache[cache_key]
            else:
                mask = self.calculate_mask(x_size, x.device)
...
class SwinIR(nn.Module):
    ...
    def clear_mask_cache(self):
        """Clear mask caches in all SwinTransformerBlocks to free memory."""
        for layer in self.layers:
            if hasattr(layer, 'residual_group'):
                for block in layer.residual_group.blocks:
                    if hasattr(block, 'clear_mask_cache'):
                        block.clear_mask_cache()
