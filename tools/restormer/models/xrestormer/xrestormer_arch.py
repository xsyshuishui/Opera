## XRestormer: Enhanced Restormer with Dual Attention
## Adapted from https://github.com/swz30/X-Restormer
## Modified for integration with Chain training framework
...
class OCAB(nn.Module):
    ...
    def forward(self, x):
        ...
        # Dynamic padding: ensure h and w are divisible by window_size
        pad_h = (self.window_size - h % self.window_size) % self.window_size
        pad_w = (self.window_size - w % self.window_size) % self.window_size
        if pad_h > 0 or pad_w > 0:
            # Use 'replicate' mode as 'reflect' fails when pad_size >= input_size
        ...
        out = rearrange(out, '(b h w head) (p1 p2) c -> b (head c) (h p1) (w p2)', head = self.num_spatial_heads, h = h_pad // self.window_size, w = w_pad // self.window_size, p1 = self.window_size, p2 = self.window_size)
