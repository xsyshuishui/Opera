...
# Add restormer source directory to path
sys.path.insert(0, str(Path(__file__).parent.parent / "restormer"))
...
MODEL_MAP = {
    ...
    "brighten.gamma_correction": "cv2.gamma_correction",
    "brighten.constant_shift": "cv2.constant_shift"
}
...
class CV2BrightenTool:
    """
    OpenCV 工具的包装类。
    用于伪装成 PyTorch 模型，使传统图像处理函数能够直接在深度学习 pipeline 中流转。
    """
    def __init__(self, tool_name):
        self.name = tool_name
        # 核心：将 net_g 指向自身，使得后续代码调用 model.net_g(x) 时直接触发 __call__
        self.net_g = self  

    def eval(self):
        pass  # 兼容 PyTorch 的 model.eval()

    def to(self, device):
        return self  # 兼容 model.to(DEVICE)

    def __call__(self, x):
        """
        处理张量。
        Args:
            x: 张量形状 (1, C, H, W)，值域 [0, 1]
        Returns:
            输出同格式张量，值域 [0, 1]
        """
        device = x.device
        
        # 1. 手动 Tensor [0, 1] -> Numpy BGR [0, 255] (摆脱隐式依赖)
        x_cpu = x.squeeze(0).cpu().clamp(0, 1).numpy()  # (C, H, W)
        x_rgb = np.transpose(x_cpu, (1, 2, 0))          # (H, W, C)
        img_bgr = cv2.cvtColor((x_rgb * 255.0).astype(np.uint8), cv2.COLOR_RGB2BGR)
        ...
        # 3. 核心提亮逻辑
        if "gamma_correction" in self.name:
            gamma = 1.5
            v = cv2.pow(v / 255.0, 1.0 / gamma) * 255
            v = np.clip(v, 0, 255).astype(np.uint8)
        elif "constant_shift" in self.name:
            shift = 40
            v = np.clip(v.astype(np.uint16) + shift, 0, 255).astype(np.uint8)

        # 4. 合并通道并转回 BGR
        hsv = cv2.merge((h, s, v))
        out_bgr = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)

        # 5. 手动 Numpy BGR [0, 255] -> RGB Tensor [0, 1] (强制除以 255.0)
        out_rgb = cv2.cvtColor(out_bgr, cv2.COLOR_BGR2RGB)
        out_tensor = torch.from_numpy(out_rgb.transpose(2, 0, 1)).float() / 255.0

        return out_tensor.unsqueeze(0).to(device)
...
def initialize_models(checkpoint_dir=None):
    ...
    #  brighten tools
    models_dict["cv2.gamma_correction"] = CV2BrightenTool("cv2.gamma_correction")
    models_dict["cv2.constant_shift"] = CV2BrightenTool("cv2.constant_shift")
    print("  Loaded cv2.gamma_correction")
    print("  Loaded cv2.constant_shift")
